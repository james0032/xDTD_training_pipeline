...
if __name__ == "__main__":
    ...
    logger.info("Read embedding file")
    ...
    logger.info("Generate node label vector")
    ...
    logger.info("Use part of data as validation data")
    ...
    logger.info("Generate Graph json file")
    data = {'directed': False,
    ...
    #         out_res = [elem for elem in executor.map(initialize_node, out_iters)]
    # else:
    ...
    out_res = pd.DataFrame({
        'test': [False] * len(entity2id),
        'id': list(range(len(entity2id))),
        'feature': [[]] * len(entity2id),
        'label': [[]] * len(entity2id),
        'val': [False] * len(entity2id)
    })
    out_res.loc[valid,'val'] = True
    data['nodes'] = out_res.to_dict('records')
    ...
    graph_data['test_removed'] = False
    graph_data['train_removed'] = False
    out_res = graph_data[['test_removed','train_removed','id1','id2']]
    out_res.columns = ['test_removed','train_removed','source','target']
    data['links'] = out_res.to_dict('records')
    ...
    logger.info("Generate class_label")
    class_map = {str(i-1):nodeclass_data_index[id2entity[i]][1] for i in tqdm(list(entity2id.values()))}
    #save labels
    with open(os.path.join(args.output_folder, 'data-class_map.json'),'w') as f:
        f.write(json.dumps(class_map))

    #generate id_label
    logger.info("Generate id_label")
    id_map = {str(i-1):(i-1) for i in tqdm(list(entity2id.values()))}
    #save nodes
    with open(os.path.join(args.output_folder, 'data-id_map.json'),'w') as f:
        f.write(json.dumps(id_map))

    #generate feats.npy
    logger.info("Generate feats.npy")
    if emb_file is not None:
        feats = np.array([emb_file[id2entity[i]] for i in tqdm(list(entity2id.values()))])
